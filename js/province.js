    ...
    $.getJSON("data/provincias.geojson", function (provincias) {
        ...
        $.getJSON("data/municipios.geojson", function (municipios) {
            ...
            $.getJSON("data/covid19-cuba.json", function (data) {
                let max = 0, total = 0, dates = [];
                for (const i in data.casos.dias) {
                    dates.push(data.casos.dias[i].fecha);
                ...
                $('#update').html(dates[dates.length - 1]);

                var locations = L.geoJSON($.walker.municipality.list, {style: styleM});
