...
$.walker = {
  ...
  view: {
    ...
    update: function () {
      ...
      if ($locator.val() !== 'cuba') {
        $selector_span.html(
          'Distribución de fallecidos por municipios en ' +
      ...
      let $generals = $(
        '#recdist, #deadist, #tesmade-pcr, #tesacum, #topprov, #compari, #topn-n-countries, #evomade, #proscurves, #testspor, #stringencycub, #casinfo, #actalt'
      );
      if (general_view) {
    ...
    $.walker.load('data/covid19-fallecidos.json', function (data) {
      $.walker.load('data/provincias.geojson', function (provincias) {
        ...
        $.walker.load('data/municipios.geojson', function (municipios) {
          ...
          let province_code = provinces_codes[province_id];
          console.log(muns);
          ...
          function getAllCasesAndSimpleGraphics() {
              ...
              if ('fallecidos' in data.casos.dias[day]) {
                ...
                for (var p in dead) {
                  ...
                  if (!(dead[p].dpacode_municipio_deteccion in muns)) {
                    if (dead[p].dpacode_provincia_deteccion !== province_code) {
                      continue;
                    }
                    console.log(dead[p]);
                    shouldAddToMun = false;
                  }
                  if (shouldAddToMun) {
            ...
            morb_a.sort(function compare(kv1, kv2) {
              return kv2[1] - kv1[1];
            });
            ...
            for (var i = 1; i <= Object.keys(data.casos.dias).length; i++) {
              ...
              if ('fallecidos' in data.casos.dias[i]) {
                ...
                for (const j in data.casos.dias[i].fallecidos) {
                  if (
                    data.casos.dias[i].fallecidos[j]
                    ...
                    data.casos.dias[i].fallecidos[j]
                      .dpacode_provincia_deteccion === province_code
                  ) {
                    report_day++;
                  }
                  if (
            ...
            c3.generate({
              ...
              data: {
                ...
                groups: [['Fallecidos Hombres', 'Fallecidos Mujeres']],
                type: 'bar',
                colors: {
          ...
          $(pros_array.slice(0, MAX_LISTS)).each(function (index, item) {
            var row = (
              ...
              .replace(
                '{tasa}',
                ((item.total * 100000) / population[item.cod]).toFixed(2)
              );
          ...
          function getMunProfile(code, mun, pro) {
            ...
            if (code in muns) {
              if (muns[code].total)
                t +=
                  '<div class="small-content"><span class="bd">Fallecidos:</span> <span>' +
                  muns[code].total +
                  '</span></div>';
              else t += '<div class="small-content">No hay fallecidos</div>';
            }
          ...
          function getProProfile(code, pro) {
            ...
            if (code in pros) {
              if (pros[code].total)
                ...
                t +=
                  '<div class="small-content">Sin fallecidos reportados aún</div>';
            }
            t += '<div class="small-plink">&nbsp;</div>';
...
let $cards = $(
  '[data-content=activo],[data-content=evacua],[data-content=recupe]'
).parent();
$locator
  .change(function () {
    $.walker.view.update();
